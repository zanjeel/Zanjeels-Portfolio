    ...
    const Ballpit = ({ className = '', followCursor = true, ...props }) => {
      ...
      useEffect(() => {
        ...
        try {
          ...
          setError('Failed to initialize 3D animation');
        }
      ...
      if (error) {
        return (
          <div className={`${className} w-full h-full flex items-center justify-center bg-gray-900/50 rounded-lg`}>
            <div className="text-center p-4">
              <p className="text-gray-400 text-sm">{error}</p>
              <p className="text-gray-500 text-xs mt-2">Please try using a modern browser with WebGL support</p>
            </div>
          </div>
        );
      }
