...
const ClientMetallicPaint = ({
    ...
    function updateUniforms() {
        if (!gl || !uniforms) {
            console.log("Missing gl or uniforms in updateUniforms");
            return;
        }
        console.log("Updating uniforms with params:", params);
    ...
    useEffect(() => {
        ...
        function initShader() {
            ...
            // Add logging for shader compilation
            ...
            // Add logging for program linking
            if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    ...
    // Only run effects when we're on the client side
    useEffect(() => {
        if (!isClient || !gl || !uniforms) return;
        updateUniforms();
    }, [isClient, gl, params, uniforms]);
    ...
    }, [isClient, gl, uniforms, imageData]);
...
// Export parseLogoImage as a client-side only function
export const parseLogoImage = async (file) => {
...
// Export the dynamically loaded version
export default dynamic(() => Promise.resolve(ClientMetallicPaint), {
    ssr: false
});
